...
sed -e "s|%DISKSIZE%|$IMAGE_SIZE|g"  \
    -e "s|%NAME%|$IMAGE_NAME|g" $DIR/centos6-minimal.tdl.template > $TDL

oz-install -d3 -u $TDL -x $POOL_PATH/$IMAGE_NAME.xml -p -a $DIR/centos6-minimal.auto -c $OZ_CONFIG
